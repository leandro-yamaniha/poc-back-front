# Documentation Directory

Este diretório contém toda a documentação detalhada do projeto Beauty Salon Management System.

## 📚 Índice de Documentação

### 🚀 **Deployment e Instalação**
- **[DEPLOYMENT_GUIDE.md](DEPLOYMENT_GUIDE.md)** - Guia completo de deploy
- **[INSTALLATION_GUIDE.md](INSTALLATION_GUIDE.md)** - Guia de instalação do projeto
- **[DOTNET_INSTALLATION_GUIDE.md](DOTNET_INSTALLATION_GUIDE.md)** - Instalação específica do .NET Core

### 🐳 **Docker**
- **[DOCKER_COMPOSE_GUIDE.md](DOCKER_COMPOSE_GUIDE.md)** - Guia de uso do Docker Compose
- **[DOCKER_COMPOSE_ANALYSIS.md](DOCKER_COMPOSE_ANALYSIS.md)** - Análise dos arquivos Docker Compose

### 🛠️ **Ferramentas e Desenvolvimento**
- **[DEVELOPMENT_TOOLS_GUIDE.md](DEVELOPMENT_TOOLS_GUIDE.md)** - Ferramentas de desenvolvimento
- **[copilot-instructions.md](copilot-instructions.md)** - Instruções para GitHub Copilot

### 📊 **Qualidade e Performance**
- **[SONAR_SETUP_GUIDE.md](SONAR_SETUP_GUIDE.md)** - Configuração do SonarQube
- **[SONAR_LOCAL_SETUP_GUIDE.md](SONAR_LOCAL_SETUP_GUIDE.md)** - SonarQube local
- **[PERFORMANCE_TEST_RESULTS.md](PERFORMANCE_TEST_RESULTS.md)** - Resultados de testes de performance

### 📝 **Status e Relatórios**
- **[FINAL_STATUS_REPORT.md](FINAL_STATUS_REPORT.md)** - Relatório final do projeto

---

## 🎯 Documentação por Categoria

### Para Começar
1. [INSTALLATION_GUIDE.md](INSTALLATION_GUIDE.md) - Instale o projeto
2. [DOCKER_COMPOSE_GUIDE.md](DOCKER_COMPOSE_GUIDE.md) - Execute com Docker
3. [DEVELOPMENT_TOOLS_GUIDE.md](DEVELOPMENT_TOOLS_GUIDE.md) - Configure ferramentas

### Para Deploy
1. [DEPLOYMENT_GUIDE.md](DEPLOYMENT_GUIDE.md) - Guia completo
2. [DOCKER_COMPOSE_ANALYSIS.md](DOCKER_COMPOSE_ANALYSIS.md) - Entenda estrutura

### Para Qualidade
1. [SONAR_SETUP_GUIDE.md](SONAR_SETUP_GUIDE.md) - Configure análise
2. [PERFORMANCE_TEST_RESULTS.md](PERFORMANCE_TEST_RESULTS.md) - Veja métricas

---

## 📖 Documentação Específica

### Backend
Para documentação específica dos backends, consulte:
- **[../backend/docs/README.md](../backend/docs/README.md)** - Documentação de backends
  - Comparação entre backends
  - Conquistas do backend reativo
  - Guias técnicos específicos

---

## 📖 Documentação Principal

A documentação principal do projeto está no arquivo **[README.md](../README.md)** na raiz do projeto.

---

## 🔗 Links Rápidos

- **Raiz**: [../README.md](../README.md)
- **Backend**: [../backend/README.md](../backend/README.md)
- **Backend Docs**: [../backend/docs/README.md](../backend/docs/README.md)
- **Frontend**: [../frontend/README.md](../frontend/README.md)

---

**📚 Toda documentação detalhada está organizada neste diretório!**
